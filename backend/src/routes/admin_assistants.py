...
def get_assistants(college: str):
    ...
    try:
        ...
        with get_dict_cursor(conn) as cur:
            # Join with labs to get assigned lab numbers
            cur.execute("""
                SELECT
                    a.id,
                    a.name,
                    ...
                    a.phone,
                    a.password,
                    CAST(l.floor AS TEXT) AS "assignedLabNo"
                FROM assistants a
...
def create_assistant(college: str):
    ...
    # We require a lab_id by schema, default to the first lab or handle gracefully
    # Realistically we should allow creating an assistant WITHOUT a lab and assign later,
    # but the schema says `lab_id INT NOT NULL`.
    # To fix this without schema migrations, we'll try to find any valid lab,
    # or the frontend must send a lab_id.
    ...
    try:
        conn = get_db_connection()
        with get_dict_cursor(conn) as cur:
            # If no lab_id provided, find the first available lab as a dummy fallback to satisfy schema
            if not lab_id:
